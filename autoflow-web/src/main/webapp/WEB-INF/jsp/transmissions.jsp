...
<html>
<head>
    <title>Управление трансмиссиями</title>
    <meta charset="UTF-8">
    <style>
        body {
...
<div class="container">
    ...
    <form method="post" action="transmissions">
    ...
    <div class="search-form">
        <form method="get" action="transmissions">
            <label>
                Поиск:
    ...
    <table>
        ...
        <jsp:useBean id="result" scope="request" type="com.kargin.autoflow.util.PaginationHelper<com.kargin.autoflow.entity.Transmission>"/>
        <c:forEach var="item" items="${result.items}">
            <tr>
                <td>${item.id}</td>
                <td>${item.type}</td>
                <td>${item.serialNumber}</td>
                <td>${item.hasCar ? 'Используется' : 'Используется'}</td>
                <td>
                    <a href="transmissions?action=delete&id=${item.id}" class="btn-danger"
    ...
    <div class="pagination">
        <c:if test="${result.hasPreviousPage}">
            <a href="transmissions?page=${result.currentPage - 1}&search=${params.search}&sortBy=${params.sortBy}&sortOrder=${params.sortOrder}">Предыдущая</a>
        </c:if>
        <span>Страница ${result.currentPage} из ${result.totalPages} (Всего: ${result.totalItems})</span>
        <c:if test="${result.hasNextPage}">
            <a href="transmissions?page=${result.currentPage + 1}&search=${params.search}&sortBy=${params.sortBy}&sortOrder=${params.sortOrder}">Следующая</a>
        </c:if>
    </div>
</div>
</body>
